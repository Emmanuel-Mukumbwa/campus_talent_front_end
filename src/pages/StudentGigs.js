...
import { Container, Row, Col, Button, Offcanvas, Spinner, Alert } from 'react-bootstrap';
...
export default function StudentGigs() {
  ...
  // Load mock analytics
  useEffect(() => {
    import('../data/gigs').then(({ studentAnalytics }) => {
  ...
  const handleAction = (gig, action) => {
    switch (action) {
      case 'apply':
        navigate(`/gigs1/apply/${gig.id}`);
        break;
      case 'save':
        console.log('Saving gig:', gig.id);
        break;
      default:
      ...
      <Row>
        ...
        <Col lg={6} xs={12}>
          ...
          {loading ? (
            ...
            <GigsGrid gigs={gigs} role="student" columns={2} onAction={handleAction} />
          )}
        </Col> 

        {/* Right: Analytics */}
        <Col lg={3} className="d-none d-lg-block">
          <AnalyticsPanel role="student" analytics={analytics} />
        </Col>
      </Row>
    </Container>
  );
