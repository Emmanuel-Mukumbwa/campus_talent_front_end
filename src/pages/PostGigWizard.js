// File: src/pages/PostGigWizard.jsx
...
const PostGigWizard = () => {
  ...
  // attach JWT header automatically; load user; compute minExpiry;
  // and if gigId is present, fetch existing gig to prefill
  useEffect(() => {
      ...
      api.get(`/api/gigs1/${gigId}`)
        .then(({ data }) => {
          setGig({
            ...
            customType: '',
            selectedSkills: data.skills.map(s => s.skill),
        ...
        .catch(() => alert('Failed to load gig for editing.'));
    }
  }, [gigId]);

  // upsert helper: draft or open; uses PUT if editing
  const upsert = async status => {
    const payload = {
      ...
      payment_amount:
        gig.budgetType === 'Paid' ? gig.paymentAmount : null,
      payment_method:
        gig.budgetType === 'Paid' ? gig.paymentMethod : null,
      bank_account_number:
    ...
    if (gigId) {
      // EDITING: use PUT on new gigs1 endpoint
      await api.put(`/api/gigs1/${gigId}`, payload);
    } else {
      // CREATING: original POST
      const { data } = await api.post('/api/recruiter/gigs', payload);
      if (data.gigId) {
        setGig(prev => ({ ...prev, id: data.gigId }));
      }
    }
  };
  ...
  const isValid = () => {
    if (step === 2) return Object.keys(validate()).length === 0;
    return true;
  };

  const handleNext = async () => {
    // AUTH GUARD at step 1
    if (step === 1) {
      if (!isAuthenticated) {
        setAuthModalMsg('You must be logged in as a Recruiter to post a gig.');
        return setShowAuthModal(true);
      }
      if (userRole !== 'recruiter') {
        setAuthModalMsg('Only users with the Recruiter role can post gigs.');
        return setShowAuthModal(true);
      }
    }
    ...
    try {
      await upsert(step < TOTAL_STEPS ? 'draft' : 'open');
          ...
          <div className="wizard-card">
            ...
            <div className="mt-3">
              ...
              <select
                ...
                value={gig.gigType}
                onChange={e => setGig({ ...gig, gigType: e.target.value, customType: '' })}
              >
                <option value="">Select…</option>
                {['Project', 'Internship', 'Part-Time'].map(o => (
              ...
              {gig.gigType === 'Other' && (
                <input
                  ...
                  onChange={e => setGig({ ...gig, customType: e.target.value })}
                />
              )}
              {errors.gigType && <div className="invalid-feedback">{errors.gigType}</div>}
            </div>
            ...
            <div className="mt-3">
              ...
              {errors.selectedSkills && <div className="text-danger mt-1">{errors.selectedSkills}</div>}
            </div>
            ...
            <div className="mt-3">
              ...
              <select
                ...
                onChange={e => setGig({ ...gig, budgetType: e.target.value })}
              >
                <option value="">Select…</option>
              ...
              {errors.budgetType && <div className="invalid-feedback">{errors.budgetType}</div>}
            </div>
            {gig.budgetType === 'Paid' && (
              <div className="row mt-3">
                <div className="col-md-4">
                  ...
                  <input
                    ...
                    onChange={e => setGig({ ...gig, paymentAmount: e.target.value })}
                  />
                ...
                <div className="col-md-4">
                  ...
                  <select
                    ...
                    onChange={e => setGig({ ...gig, paymentMethod: e.target.value })}
                  >
                    <option value="">Select…</option>
                  ...
                  <>
                    <div className="col-md-4 mt-3 mt-md-0">
                      ...
                      <input
                        ...
                        onChange={e => setGig({ ...gig, bankAccountNumber: e.target.value })}
                      />
                    ...
                    <div className="col-md-4 mt-3 mt-md-0">
                      ...
                      <input
                        ...
                        value={gig.bankName}
                        onChange={e => setGig({ ...gig, bankName: e.target.value })}
                      />
      ...
      case 2:
        return (
          <div className="wizard-card">
            ...
            <ul className="mt-3">
              {Object.entries(validate()).map(([field, msg]) => <li key={field}>{msg}</li>)}
            </ul>
          </div>
        );
      case 3:
        return (
          <div className="wizard-card">
            ...
            <dl className="mt-3">
              ...
              <dt>Type</dt><dd>{gig.gigType === 'Other' ? gig.customType : gig.gigType}</dd>
            ...
            <div className="alert alert-info">
              ...
              Ready to go live? Click “{gigId ? 'Save Gig' : 'Publish Gig'}.”
            </div>
          </div>
        );
      default:
  ...
  return (
    <div className="container py-5">
      ...
      <div className="text-center mb-4">
        <h2 className="text-success">{gigId ? 'Edit Gig' : 'Post a New Gig'}</h2>
      ...
      <main>
        ...
        <div className="d-flex justify-content-between mt-4">
          ...
          <button className="btn btn-success" onClick={handleNext} disabled={step === 2 && !isValid()}>
            {step < TOTAL_STEPS ? 'Next' : (gigId ? 'Save Gig' : 'Publish Gig')}
          </button>
        </div>
      </main>
    </div>
