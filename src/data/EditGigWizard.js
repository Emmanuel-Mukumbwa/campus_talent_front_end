// File: src/pages/EditGigWizard.jsx
...
const EditGigWizard = () => {
  ...
  useEffect(() => {
    ...
    if (gigId) {
      api.get(`/api/recruiter/gigs1/${gigId}`)
        .then(({ data }) => {
          setGig({
            id: data.id,
            title: data.title,
  ...
  const upsert = async status => {
    const payload = {
      ...
      payment_amount: gig.budgetType === 'Paid' ? gig.paymentAmount : null,
      payment_method: gig.budgetType === 'Paid' ? gig.paymentMethod : null,
      bank_account_number:
        gig.paymentMethod === 'BankAccount' ? gig.bankAccountNumber : null,
      bank_name:
    ...
    await api.put(`/api/recruiter/gigs1/${gigId}`, payload);
  };
  ...
  const validate = () => {
    ...
    if (step === 1) {
      ...
      if (!(gig.gigType || gig.customType.trim())) e.gigType = 'Type is required';
  ...
  const isValid = () => (step === 2 ? Object.keys(validate()).length === 0 : true);

  const handleNext = async () => {
    if (step === 1) {
      if (!isAuthenticated) {
        setAuthModalMsg('You must be logged in as a Recruiter to edit a gig.');
        setShowAuthModal(true);
        return;
      }
      if (userRole !== 'recruiter') {
        setAuthModalMsg('Only recruiters may edit gigs.');
        setShowAuthModal(true);
        return;
      }
    }
    ...
    try {
      ...
      if (step < TOTAL_STEPS) {
        ...
        setSuccessMsg('Your gig has been updated successfully!');
          ...
          <div className="wizard-card">
            ...
            <div className="mt-3">
              ...
              >
                ...
                {['Project', 'Internship', 'Part-Time'].map(o => <option key={o}>{o}</option>)}
            ...
            <div className="mt-3">
              ...
              >
                <option value="">Select…</option>
                {['Paid', 'Academic Credit', 'Volunteer'].map(o => <option key={o}>{o}</option>)}
              </select>
        ...
        return (
          <div className="wizard-card">
            ...
            <dl className="mt-3">
              ...
              <dt>Skills</dt>
              <dd>{gig.selectedSkills.join(', ')}</dd>
              <dt>Budget</dt>
              <dd>
                {gig.budgetType}
                {gig.budgetType === 'Paid' &&
                  ` — MWK ${gig.paymentAmount} via ${gig.paymentMethod}`}
              </dd>
              {gig.paymentMethod === 'BankAccount' && (
                <>
                  <dt>Bank Account #</dt>
                  <dd>{gig.bankAccountNumber}</dd>
                  <dt>Bank Name</dt>
                  <dd>{gig.bankName}</dd>
                </>
              )}
              ...
              <dd>{gig.duration}</dd>
              <dt>Deliverables</dt>
              <dd>{gig.deliverables}</dd>
              <dt>Contact Info</dt>
              <dd>{gig.contactInfo}</dd>
              <dt>Expires At</dt>
              <dd>{gig.expiresAt}</dd>
            </dl>
            <div className="alert alert-info">
              <FaCheckCircle className="me-1" />
              Ready to go live? Click “Save Gig.”
            </div>
          </div>
        );
      default:
  ...
  return (
    <div className="container py-5">
      ...
      <div className="text-center mb-4">
        <h2 className="text-success">Edit Gig</h2>
      ...
      <main>
        ...
        <div className="d-flex justify-content-between mt-4">
          ...
          >
            {step < TOTAL_STEPS ? 'Next' : 'Save Gig'}
          </button>
        </div>
      </main>
    </div>
